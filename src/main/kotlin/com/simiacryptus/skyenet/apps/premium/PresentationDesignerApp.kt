...
open class PresentationDesignerAgent(
  ...
  val actors = PresentationDesignerActors(
    model = model,
  ...
  @Suppress("UNCHECKED_CAST")
  private val initialAuthor by lazy { actors.get(ActorType.INITIAL_AUTHOR.name)!! as ParsedActor<Outline> }
  private val slideAuthor by lazy { actors.get(ActorType.CONTENT_EXPANDER.name)!! as SimpleActor }
  private val slideLayout by lazy { actors.get(ActorType.SLIDE_LAYOUT.name)!! as SimpleActor }
  private val slideSummary by lazy { actors.get(ActorType.SLIDE_SUMMARY.name)!! as SimpleActor }
  private val imageRenderer by lazy { actors.get(ActorType.IMAGE_RENDERER.name)!! as ImageActor }
  private val narrator get() = actors.get(ActorType.NARRATOR.name)!! as TextToSpeechActor
  ...
  fun main(userRequest: String) {
    ...
    try {
      ...
      mainTask.header("Starting Presentation Generation", 2)
  ...
  private fun slideContents(
    ...
    slideTask.apply {
      ...
      complete("<iframe src=\"${refBase}slide_$idx.html\" class='slide-content' style=\"width: 100%; height: 600px; border: none;\"></iframe>")
    }
      ...
      """
        ...
        slideContents.withIndex().joinToString("\n") { (i, slide) ->
          ui.newTask(false).apply {
            reportTabs[i.toString()] = placeholder
            complete("<iframe src=\"${refBase}slide_$i.html\" class='slide-content' style=\"width: 100%; height: 600px; border: none;\"></iframe>")
          }
          """
